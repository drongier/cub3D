...
void    ft_xpm_check(t_scene *scene, char *texture)
{
    ...
    {
        free (tmp);
        ft_error(scene, "Error: Texture files' format should be '.xpm' !");
    }
    free (tmp);
}
...
{
    int fd;
    if (!(scene->no_texture && scene->so_texture 
        && scene->we_texture && scene->ea_texture))
        ft_error(scene, "Missing a texture path!");
    ft_xpm_check(scene, scene->no_texture);
    fd = open(scene->no_texture, O_RDONLY);
    if (fd == -1)
        ft_error(scene, "NO texture file doesn't exist or have the read permission");
    close(fd);
    ft_xpm_check(scene, scene->so_texture);
    fd = open(scene->so_texture, O_RDONLY);
    if (fd == -1)
        ft_error(scene, "SO texture file doesn't exist or have the read permission");
    close(fd);
    ft_xpm_check(scene, scene->we_texture);
    fd = open(scene->we_texture, O_RDONLY);
    if (fd == -1)
        ft_error(scene, "WE texture file doesn't exist or have the read permission");
    close(fd);
    ft_xpm_check(scene, scene->ea_texture);    
    fd = open(scene->ea_texture, O_RDONLY);
    if (fd == -1)
        ft_error(scene, "EA texture file doesn't exist or have the read permission");       
    close(fd);
    // ft_xpm_check(scene);
}
...
void	ft_config_file_check(char *arg)
{
	...
	close (fd);
    if (fd == -1)
	 	ft_error(0, "Config file doesn't exist or have the read permission!");
    tmp = ft_strtrim(arg, " \t");
    s = ft_strrchr(tmp, '.');
